...
function prepareProductionLedgerCandidate(){return prepareLedger_(REHEARSAL_SPREADSHEET_ID,rehearsalOwner_());}
function runProductionMigrationRehearsal(){const backup=latestMatchingRawBackup_(),started=Date.now(),result=migrateLegacy_(REHEARSAL_SPREADSHEET_ID,backup.file.getId(),rehearsalOwner_());return{backupFileId:backup.file.getId(),backupChecksum:backup.value.checksum,elapsedMs:Date.now()-started,result:result,sourceOfTruth:PropertiesService.getScriptProperties().getProperty(LEDGER_POINTER)?'Google Sheets':'swtm2'};}
function verifyProductionMigrationRehearsal(){
  ...
  return{source:source.original,target:integrity_(target),expectedTarget:integrity_(expected),countsMatch:canonical_(source.target.counts)===canonical_(integrity_(target).counts),checksumMatch:checksum_(expected)===checksum_(target),relationIntegrity:validated,reviewRequired:source.uncertain,readMs:readMs,sourceOfTruth:PropertiesService.getScriptProperties().getProperty(LEDGER_POINTER)?'Google Sheets':'swtm2'};
}
function runCandidateReadWriteRoundTrip(){
  return withLock_(function(){
    ...
    return{changedReadback:true,restored:true,beforeChecksum:beforeChecksum,restoredChecksum:restoredChecksum,writeMs:writeMs,restoreMs:restoreMs};
  });
}
